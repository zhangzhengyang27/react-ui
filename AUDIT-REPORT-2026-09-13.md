...
> ⑮ 第十五批（dates + form/pro P1）已完成：Calendar 无 date 时的兜底月份改用 getDefaultClampedDate（此前钳制条件写反，minDate 在过去时打开在 minDate 月份）；DateTimePicker 关闭双 onChange 修复——popover 关闭路径只钳制不清理（handleClose 已清理），钳制块对未完成区间跳过（顺带修掉"钳制复活已清空值"）；InlineDateTimePicker 对 type="multiple" 显式拒绝并 warn（此前点选即把数组破坏成字符串、下一帧崩溃）；pro/SearchFilter 查询按钮透传 event（onSearch 双触发修复）；ProTable 渲染期 assignRef 移入 effect + 重置接线 onReset→handleSearch（重置后表格同步回到默认筛选）；form/getInputOnChange 对 file 输入存 FileList（此前存 fakepath 字符串）；form 提交级 validate 开始时 abortFieldValidations 中止全部在途字段级异步校验（过期结果不再污染提交后错误状态）。**核实后不修**：ProTable 内联 request 无限循环——仓库测试编码了"切换 request 函数即重新请求"的语义，与防循环互斥，以测试为准（消费方内联函数需自行 useCallback，已在注释说明）。验证：dates 1433、form 409、pro 17 全部通过。
> ⑯ 第十六批（ui/hooks P1 长尾 + 低成本 P2）已完成：Watermark 瓦片按 canvas measureText 实测文本宽度与旋转投影取尺寸（长文本不再被 SVG 裁切出残缺水印），gap 依赖改原始值（内联数组不再每渲染重建 data URL）；DataTable 行级 Enter/Space 不再拦截行内交互元素（展开按钮/复选框/链接的键盘激活与 onRowClick 误触修复）；Cascader 面板与搜索项补 onMouseDown preventDefault（多选模式点选项不再被 onBlur 误关面板），搜索项 aria-selected 按真实选中态渲染（此前恒 false）；use-scroll-into-view 的 cancel() 同步重置计时起点（取消后再次滚动不再瞬移到目标）；use-floating-window 补 touchcancel 监听 + 拖拽中卸载恢复 body userSelect（触摸中断/卸载不再全局卡死选中）+ effect 依赖补 excludeDragHandleSelector；低成本 P2：MenubarMenu 解析后不再每渲染全量 querySelectorAll、Progress aria-valuenow 钳制、Alert 关闭按钮默认 aria-label、ScrollArea 竖向滚动条 bottom 误用 corner-width 改 corner-height、TableOfContents minDepthToOffset || 1 改 ?? 1。验证：ui 528、hooks 110 全部通过，tsc 通过。
> ⑰ 第十七批（按用户决策：最后一批）已完成：
> - ModalStack/DrawerStack 接线（决策：接线）：Modal/Drawer 在 Stack 内挂载时向栈注册（addModal/removeModal），zIndex 由栈按挂载顺序经 getZIndex 递增分配，嵌套弹层自动层叠；Modal 补 id 透传（注册与 ModalBase 共用同一 id）。
> - Rating 重构为 radio 组（决策：重构）：隐藏原生 radio 承载键盘导航与表单语义（role=radiogroup，方向键原生移动、roving tabindex），星形视觉由 label 呈现；hover 半星预览与 click 清零逻辑保留（clearable 经已选中 radio 的 click 事件实现）；新增 name prop；测试更新为 radio 语义并补勾选态断言。
> - hooks 防御性 ref 化（决策：做）：use-selection 内联 data 浅比较（不再每渲染清空选择）；use-local-storage/session-storage serialize/deserialize 走 ref（不再固化首帧闭包）；use-event-listener options 走 ref（内联对象不再每渲染解绑重绑，capture 位按绑定时记录）；use-mutation-observer/use-mutation-observer-target callback/options 走 ref（不再每渲染 disconnect/重建 observer）。
> - 全量验证：hooks/pro/ui 构建成功（schedule/dates/form 为源码直引包无构建产物），ui tsc 通过，11 个包测试全绿：ui 529（+1）、schedule 1349、dates 1433、form 409、hooks 110、dropzone 61、code-highlight 70、carousel 45、spotlight 25、pro 17、modals 7。
> ⑱ 第十八批（P2 清尾，一次性清完剩余项）已完成：
> - **Combobox 系**：MultiSelect 胶囊区点击阻断冒泡（下拉展开时点胶囊不再被误关，聚焦行为保留）；pills-reorder 的 findSearchInput 向上扩一层查找（末尾胶囊 ArrowRight 焦点死路修复）；Autocomplete 清除按钮补 mousedown preventDefault + selectedValues 稳定引用（不再击穿 contextValue memo）+ 选项 key 附索引；ComboboxPopover 选项 key 附索引、withScrollArea 默认值补齐（对齐 @default true 文档）；Transfer 搜索保留 ReactNode label 条目并支持按 value 匹配。
> - **Tree/Cascader**：expandAllNodes 一次性遍历替代逐 key findTreeNode（O(n²)→O(n)）；Cascader 搜索 collectLeafMatches 携带父链构造 path（O(n²)→O(n)）；loadData 失败 catch + dev 告警（不再 unhandled rejection）；TreeNode 补 aria-expanded；TreeSelect onBlur relatedTarget 守卫（点 Pill 移除不再误关面板）；TagsInput 粘贴按全部命中分隔符切分。
> - **schedule**：DragContext 的 dropTarget/setDropTarget 接通 hook 内真实状态（死 API 修复）；responsive 布局 ResizeObserver 跟踪容器宽度（600px 断点与 @container 一致）只挂载激活侧视图（事件计算不再翻倍）；WeekView all-day 槽 static 模式退出 Tab 序；MobileMonthView 全天判定复用 isAllDayEvent + 零长度事件按全天；MonthView 方向键遇占位格继续跳转（跳过逻辑可达）；周/日程视图事件结束秒级判定与月视图对齐（含 second）。
> - **dates**：Calendar `y` 快捷键经 clampLevel（不再绕过 maxLevel）；useUncontrolledDates 切 type 时 defaultValue 形状映射（default↔range/multiple 不再崩溃）；Day 的 data-today 加 suppressHydrationWarning（SSR 属性级水合告警抑制）；MiniCalendar 未受控锚点日 ref 惰性求值 + getDefaultClampedDate 钳制（跨午夜不固化旧值）。
> - **ui 散点**：CopyButton 透出 clipboard error；ModalsProvider 事件对象 memo 化（不再每渲染重挂 8 个 window 监听）；emotion create-styles cssObject 按 theme/params memo；SpotlightRoot disabled 时不注册快捷键；code-highlight loadContext 竞态守卫 + rejection 兜底；tiptap Mod-k 改为携带 editor 实例的 CustomEvent（多编辑器同页互不干扰）+ 源码模式双向 emitUpdate；DataTable 函数 accessor 列禁用本地排序（dev 告警）+ 吸顶固定列表头 z-index 修正；ScrollAreaAutosize attributes 不再泄漏到 DOM（透传内部 ScrollArea）；Grid/SimpleGrid 断点按数值升序生成 @media（复用 getSortedBreakpoints）；Splitter aria-valuemin/max 与实际钳位一致（5/95）；Stepper 移除未实现的 tablist/tab 角色；TimelineItem 脱离 Timeline 给出可读错误而非裸 TypeError。
> - **hooks**：use-intersection options 走 ref（threshold 内联数组不再重建 observer）；use-merged-ref/mergeRefs 清理路径对未返回 cleanup 的函数 ref 补 null 调用（React 19 下不再滞留已分离节点）；use-long-press 触摸后 500ms 内的仿真 mouse 事件跳过（不再重复 onStart/误报 onCancel）；use-focus-trap hidden() 用 checkVisibility（class/visibility 隐藏元素不再被误判可聚焦）。
