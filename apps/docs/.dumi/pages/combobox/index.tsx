...
export default function ComboboxPage() {
  ...
  const filteredData = COMBOBOX_EXAMPLES_DATA.filter((item) => {
    const splittedName = item.name.toLowerCase().split(' ').filter((p) => p.trim().length > 0);
    const splittedDescription = item.description.toLowerCase().split(' ').filter((p) => p.trim().length > 0);
