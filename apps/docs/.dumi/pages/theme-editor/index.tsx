...
import { AiOutlineCopy, AiOutlineReload, AiOutlineSave } from 'react-icons/ai';
...
const locales = {
  cn: {
    ...
    noteTitle: '说明',
    note:
      '本编辑器为简化版本，仅覆盖 react-ui 主题系统的常用字段。完整字段请参考 packages/ui/src/core/UIProvider/theme.types.ts。',
  },
  en: {
    title: 'Theme Editor',
    ...
    noteTitle: 'Note',
    note:
      'This is a simplified editor that only covers common fields of the react-ui theme system. For the full schema, see packages/ui/src/core/UIProvider/theme.types.ts.',
  },
};
            ...
            <Stack gap="md">
              ...
              <div className={classes.grid}>
                {SIZE_OPTIONS.map((size) => (
                  <TextInput
                    key={size}
                    label={locale[`fontSize${size.charAt(0).toUpperCase()}${size.slice(1)}` as keyof typeof locale]}
            ...
            <Stack gap="md">
              ...
              <div className={classes.grid}>
                {SIZE_OPTIONS.map((size) => (
                  <TextInput
                    key={size}
                    label={locale[`radius${size.charAt(0).toUpperCase()}${size.slice(1)}` as keyof typeof locale]}
            ...
            <Stack gap="md">
              <div className={classes.grid}>
                {SIZE_OPTIONS.map((size) => (
                  <TextInput
                    key={size}
                    label={locale[`spacing${size.charAt(0).toUpperCase()}${size.slice(1)}` as keyof typeof locale]}
          ...
          <Tabs.Panel value="shadows" className={classes.panel}>
            <Stack gap="md">
              {SIZE_OPTIONS.map((size) => (
                <TextInput
                  key={size}
                  label={locale[`shadow${size.charAt(0).toUpperCase()}${size.slice(1)}` as keyof typeof locale]}
