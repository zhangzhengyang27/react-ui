...
const IconSearch: React.FC = () => {
  ...
  const groupedIcons = useMemo<Record<CategoriesKeys, IconEntry[]>>(() => {
    ...
    const groups = (Object.keys(categories) as CategoriesKeys[]).reduce<
      Record<CategoriesKeys, IconEntry[]>
    >((acc, key) => {
      acc[key] = [];
      return acc;
  ...
  const hasAnyIcon = (Object.keys(groupedIcons) as CategoriesKeys[]).some(
    (key) => groupedIcons[key].length > 0,
  );
      ...
      <div id="list-of-icons">
        {hasAnyIcon ? (
          (Object.keys(groupedIcons) as CategoriesKeys[]).map((key) =>
            groupedIcons[key].length ? (
              <Category key={key} title={key} icons={groupedIcons[key]} />
            ) : null,
          )
        ) : (
          <EmptyState
