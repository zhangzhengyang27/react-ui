...
import { AiOutlineCheck, AiOutlineClose, AiOutlineCopy, AiOutlineSend } from '../../icons'
import {
    ActionIcon,
    Button,
    Divider,
...
const PromptDrawer: React.FC<PromptDrawerProps> = ({ open, onClose, onThemeChange }) => {
    ...
    const [submitPrompt, loading, prompt, resText, cancelRequest, generateErrorMessage] = usePromptTheme(onThemeChange)
    const hasGenerateError = generateErrorMessage !== undefined

    const { recommendations, loading: recommendLoading, fetch: fetchRecommendations } = usePromptRecommend(localeKey)
            ...
            {recommendLoading ? (
                ...
                recommendedPrompts.length > 0 && (
                    <div className={classes.promptsList}>
                        ...
                        <Button variant="subtle" size="sm" radius="md" onClick={handleRefreshRecommendations}>
                            🔄 {locale.refresh}
                        </Button>
                    </div>
                )
            ...
            <div className={classes.msgRow}>
                ...
                <div className={classes.aiContent}>
                    {loading && !resText && !hasGenerateError ? (
                        ...
                        <pre className={classes.codeBlock}>{hasGenerateError ? generateErrorMessage : resText}</pre>
                    )}
                    {!loading && resText && !hasGenerateError && (
