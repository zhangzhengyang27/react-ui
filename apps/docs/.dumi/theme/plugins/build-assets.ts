...
function extractEmotionStyle(html: string) {
  ...
  const cacheManager = (global as any).__ANTD_STYLE_CACHE_MANAGER_FOR_SSR__;
  if (!cacheManager || typeof cacheManager.getCacheList !== 'function') {
    return [];
  }
  const styles = cacheManager.getCacheList().map((cache) => {
