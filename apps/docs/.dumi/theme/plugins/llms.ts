...
function processMarkdownFile(
  ...
  const url = `https://react-ui.dev/${urlPath}.md`;
...
async function generateLLms(api: IApi) {
  ...
  const semanticContent = [
    '# react-ui Semantic Documentation',
    '',
  ...
  const semanticContentCn = [
    '# react-ui 组件语义化描述',
    '',
    '本文档包含了 react-ui 组件库中所有组件的语义化描述信息。',
    '',
  ...
  const fullContent = [
    '# react-ui Component Documentation',
    '',
  ...
  const llmsNavContent = [
    '# react-ui - xiaoye React UI library',
    '',
    '- react-ui, developed by xiaoye, is a React UI library that aims to provide a high-quality design language and development framework for enterprise-level backend management systems. It offers a rich set of components and design guidelines, helping developers build modern, responsive, and high-performance web applications.',
    '',
    '## Navigation',
    '',
  ...
  // Publish DESIGN.md to site root as lowercase design.md so https://react-ui.dev/design.md works
  ...
  if (fs.existsSync(designMdSource)) {
    ...
    api.logger.warn('DESIGN.md not found at repo root; https://react-ui.dev/design.md will 404.');
  }
