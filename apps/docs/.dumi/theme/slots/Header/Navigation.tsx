...
const HeaderNavigation: React.FC<NavigationProps> = (props) => {
  ...
  const navItems = [
    ...
    {
      label: (
        <Link to={utils.getLocalizedPathname('/docs/hooks/package', isZhCN, search)}>
          {locale.hooks}
        </Link>
      ),
      key: 'docs/hooks',
    ...
    {
      label: (
        <Link to="/colors-generator">{locale.tools}</Link>
      ),
  ...
  if (isMobile) {
    return (
      <nav className={classes.navInline}>
        ...
        <div className={classes.navInlineItem}>
          <button onClick={onDirectionChange} style={{ background: 'transparent', border: 0, cursor: 'pointer', padding: 0, color: 'var(--ui-color-text)' }}>
            {directionText}
          </button>
        </div>
      </nav>
