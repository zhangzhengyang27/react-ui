...
export function isZhCN(_pathname: string) {
  // 站点仅保留中文文档，始终按中文处理。
  return true;
}
...
export function getLocalizedPathname(
  ...
  if (hash?.zhCN) {
    return { pathname: `${pathname}#${hash.zhCN}`, search };
  }
 ...
 * react-ui creates a temporary preview site for each PR for convenient preview and testing.
 * Usually on platforms like surge.sh or Cloudflare Pages.
 */
export function isOfficialHost(hostname: string) {
  const officialHostnames = ['react-ui.dev'];
