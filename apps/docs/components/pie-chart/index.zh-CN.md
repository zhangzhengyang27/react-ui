...
title: PieChart
subtitle: 饼图
description: react-ui PieChart 饼图组件。
group:
  title: 图表
  order: 4
---
...
设置 `size` 属性以控制图表的宽度和高度。注意，如果设置了 `withLabels` 和 `labelPosition="outside"`，图表高度会自动增加 80px 以为标签留出空间。可通过 `h` 和 `w` [样式属性](/docs/styles/style-props) 覆盖此行为。
...
```tsx
import { PieChart } from '@xiaoye-react/ui';
