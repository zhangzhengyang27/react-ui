...
所有 `@xiaoye-react/ui` 组件都通过 `events` prop 接收事件。每个事件的类型为 `ScheduleEventData`，可以是以下三种形态之一：
...
```tsx
import type { DateTimeStringValue, EventPayload } from '@xiaoye-react/ui';
...
```tsx
import type { ScheduleSingleEventData } from '@xiaoye-react/ui';
...
```tsx
import type { ScheduleRecurringSeriesEventData } from '@xiaoye-react/ui';
...
```tsx
import type { ScheduleRecurringOverrideEventData } from '@xiaoye-react/ui';
...
```tsx
import type { ScheduleEventData } from '@xiaoye-react/ui';
...
这些类型在 `@xiaoye-react/ui` 和 `@xiaoye-react/ui` 之间共享，并从两个包中导出。
