---
category: Styles
title: ColorFunctions
subtitle: 颜色函数
description: react-ui ColorFunctions 文档。
---


## darken 和 lighten

`darken` 和 `lighten` 函数可用于调整颜色的亮度。
第一个参数接受任意格式的颜色，第二个参数为要增加/减少的亮度值。

```tsx
import { darken, lighten } from '@react-ui/ui';

lighten('#228BE6', 0.1); // lighten by 10%
// -> rgba(56, 151, 233, 1)

darken('rgb(245, 159, 0)', 0.5); // darken by 50%
// -> rgba(123, 80, 0, 1)

darken('rgba(245, 159, 0, .3)', 0.5); // darken by 50%
// -> rgba(123, 80, 0, 1, .3)

lighten('var(--ui-color-gray-4)', 0.74);
// -> color-mix(in srgb, var(--ui-color-gray-4), white 74%)
```

## alpha

`alpha` 函数将颜色转换为带有指定 alpha 通道的 rgba 格式。
它通常用于让颜色更透明。如果无法将颜色转换为 rgba 格式
（例如颜色是 CSS 变量），该函数会使用 [color-mix](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value/color-mix)。
注意 `color-mix` 在某些旧版浏览器中不受支持。你可以在 [caniuse](https://caniuse.com/mdn-css_types_color_color-mix)
上查看更多信息。

```tsx
import { alpha } from '@react-ui/ui';

alpha('#4578FC', 0.45); // -> rgba(69, 120, 252, 0.45)
alpha('var(--ui-color-gray-4)', 0.74);
// -> color-mix(in srgb, var(--ui-color-gray-4), transparent 26%)
```

## parseThemeColor

`parseThemeColor` 函数返回给定颜色的相关信息，格式如下：


`parseThemeColor` 函数可以在任何可访问 `theme` 对象的地方使用，
例如 [CSS 变量解析器](/docs/styles/css-variables)、[颜色变体解析器](/docs/theming/colors#colors-variant-resolver)
或组件主体中：

```tsx
interface ParseThemeColorResult {
  /**
   * True if the given color is a theme color. For example,
   * `blue`, `orange.9`, `pink.3` are theme colors,
   * while `#fff`, `rgba(0, 0, 0, .5)` are not
   */
  isThemeColor: boolean;

  /**
   * Key of `theme.colors` if the given color is a theme color. For example,
   * if the given color is `blue` it will be `blue`,
   * if the given color is `orange.9` it will be `orange`
   */
  color: string;

  /**
   * Resolved color value. For example,
   * if the given color is `blue.7` it will be the value of `theme.colors.blue[7]`,
   * if the given color is `#fff` it will be `#fff`
   */
  value: string;

  /**
   * If the given color is a theme color, this will be the shade of that color.
   * For example, if the given color is `blue.7` it will be `7`.
   * If the given color does not have an index or is not a theme color, then it will be `undefined`
   */
  shade: UIColorShade | undefined;

  /**
   * Color CSS variable. For example:
   * `blue.7` – `--ui-color-blue-7`,
   * `red` – `--ui-color-red-filled`,
   * `white` – `--ui-color-white`,
   * `#fff` – `undefined`
   */
  variable: CssVariable | undefined;
}
```

```tsx
import {
  UIColor,
  parseThemeColor,
  useUITheme,
} from '@react-ui/ui';

interface DemoProps {
  color: UIColor;
}

function Demo({ color }: DemoProps) {
  const theme = useUITheme();
  const parsedColor = parseThemeColor({ color, theme });

  return (
    <div
      style={{
        backgroundColor: parsedColor.isThemeColor
          ? `var(${parsedColor.variable})`
          : parsedColor.value,
      }}
    />
  );
}
```

## getThemeColor

`getThemeColor` 是 `parseThemeColor` 函数的简化版本。它接受颜色字符串作为第一个参数，
theme 对象作为第二个参数，返回解析后的颜色值或 CSS 变量：

```tsx
import { getThemeColor, useUITheme } from '@react-ui/ui';

function Demo() {
  const theme = useUITheme();

  getThemeColor('blue', theme); // -> var(--ui-color-blue-filled)
  getThemeColor('blue.7', theme); // -> var(--ui-color-blue-7)
  getThemeColor('white', theme); // -> var(--ui-color-white)
  getThemeColor('#DF78E4', theme); // -> #DF78E4
}
```

## getGradient

`getGradient` 函数将给定的 `UIGradient` 对象转换为 CSS 渐变字符串：

```tsx
import { getGradient, useUITheme } from '@react-ui/ui';

function Demo() {
  const theme = useUITheme();

  getGradient({ deg: 180, from: 'blue', to: 'cyan.7' }, theme);
  // -> `linear-gradient(180deg, var(--ui-color-blue-filled) 0%, var(--ui-color-cyan-7) 100%)`
}
```

## isLightColor

`isLightColor` 函数可用于提升文本与背景之间的对比度：

```tsx
import { Box, isLightColor } from '@react-ui/ui';

interface DemoProps {
  color: string;
}

export function Demo({ color }: DemoProps) {
  return (
    <Box bg={color} c={isLightColor(color) ? 'black' : 'white'}>
      Box with contrast text
    </Box>
  );
}
```

## luminance

`luminance` 函数返回颜色的亮度值。它可用于检查颜色对比度：

```tsx
import { luminance } from '@react-ui/ui';

luminance('#fff'); // -> 1
luminance('#000'); // -> 0
luminance('#4578FC'); // -> 0.21726425554966
```
