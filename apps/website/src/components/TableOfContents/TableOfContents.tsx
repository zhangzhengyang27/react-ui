...
import { Box, ScrollArea, Space, Text } from '@react-ui/ui';
...
export function TableOfContents({ withTabs, editPageLink }: TableOfContentsProps) {
  ...
  const getDepth = useCallback(
    (element: HTMLElement) => Number(element.getAttribute('data-order')),
    []
  );
  const getValue = useCallback(
    (element: HTMLElement) => element.getAttribute('data-heading') || '',
    []
  );
