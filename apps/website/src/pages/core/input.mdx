import { InputDemos } from '@react-ui/docs-demos';
...
**!重要提示：** 在大多数情况下，你不应在应用中直接使用 `Input`。
`Input` 是其他输入框的基础，并非为直接使用而设计。
请使用 `Input` 来创建自定义输入框。其他情况下，建议使用 [TextInput](/core/text-input/)
或其他组件。
...
`Input` 组件用作某些其他输入框的基础（如 [NativeSelect](/core/native-select/)、[TextInput](/core/text-input/)、[Textarea](/core/textarea/) 等）。
...
Input 是一个[多态组件](/guides/polymorphic)，默认根元素是 `input`，
但可以更改为任何其他元素或组件。
...
`Input.Wrapper` 组件在所有其他输入框（[TextInput](/core/text-input/)、[NativeSelect](/core/native-select/)、[Textarea](/core/textarea/) 等）
内部使用。你*不需要*用它包裹输入框，因为它已经包含在所有输入框中*。
...
使用 `inputContainer` 属性，你可以增强底层使用 `Input.Wrapper` 的输入框。
例如，当输入框获得焦点时，可以为 [TextInput](/core/text-input/) 添加 [Tooltip](/core/tooltip/)：
...
唯一的区别是输入元素是否会带有 `required` 属性。以 [TextInput](/core/text-input/) 组件为例：
...
当设置为 `true` 时，会为输入框添加红色边框。你也可以传入 React 节点以在输入框下方显示错误信息。若只想显示错误信息而不显示红色边框，请将 `error` 属性
...
当设置为 `true` 时，会为输入框添加绿色边框。你也可以传入 React 节点以在输入框下方显示成功信息。如果同时设置了 `error` 和 `success` 属性，`error` 优先：
...
你可以在 [theme](/theming/theme-object/) 上为 `Input` 和 `Input.Wrapper` 组件添加[默认属性](/theming/default-props/)。
这些默认属性会被所有底层使用 `Input` 和 `Input.Wrapper` 的输入框继承（[TextInput](/core/text-input/)、[NativeSelect](/core/native-select/)、[Textarea](/core/textarea/) 等）：
...
（[TextInput](/core/text-input/)、[Textarea](/core/textarea/)、[NumberInput](/core/number-input/)、
[Select](/core/select/)、[DateInput](/dates/date-input/) 等）。这是同时将所有输入框的 `size`、`radius`、`variant`、`withAsterisk` 或其他共享属性设置为相同值的最简单方式。
每个组件的默认属性始终优先于共享的默认属性，因此你仍然可以在需要时覆盖单个组件：

<Demo data={InputDemos.sharedDefaultProps} />

## 主题上的样式

与默认属性一样，你可以在 [theme](/theming/theme-object/) 上使用 `Input` 和 `Input.Wrapper` 的 [Styles API](/styles/styles-api/)
为所有输入框添加样式：
...
使用 `&:focus-within` 选择器来更改输入框的焦点样式。你可以通过 `classNames` 属性将其应用于单个组件，或通过 [Styles API](/styles/styles-api/)
应用于 [theme](/theming/theme-object/) 上的所有输入框。
...
`Input` 和 `Input.Wrapper` 组件支持 [Styles API](/styles/styles-api) ——
你可以使用 `classNames` 和 `styles` 属性自定义任何内部元素的样式。
...
如果你在没有关联 label 元素的情况下使用 `Input` 组件，请设置 `aria-label`：
...
当你将 `Input` 与 `Input.Wrapper` 一起使用时，需要在两个组件上都设置 `id`
以将标签和其他元素与输入框关联：
...
你可以使用 [use-id](/hooks/use-id) 生成唯一 id：
