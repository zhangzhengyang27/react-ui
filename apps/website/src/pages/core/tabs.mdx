import { FloatingIndicatorDemos, TabsDemos } from '@react-ui/docs-demos';
...
如果你不需要订阅 Tabs 的状态变化，请使用 `defaultValue`：
...
使用 [FloatingIndicator](/core/floating-indicator) 的自定义变体示例：
...
当你希望在 `Tabs.Panel` 内部渲染影响性能的组件时，这很有用。注意，
...
你可以使用 `renderRoot` 属性将 `Tabs.Tab` 渲染为 `a` 元素。这为你提供
...
当标签页过多无法容纳在可用空间中时，使用 [Scroller](/core/scroller) 组件使标签页列表可滚动：
...
如果你使用的 `Tabs.Tab` 没有文本内容，例如只有图标，请设置 `aria-label`
或使用 [VisuallyHidden](/core/visually-hidden) 组件：
  ...
  return (
    <Tabs defaultValue="chat">
      <Tabs.List>
        ...
        {/* 你可以使用 VisuallyHidden 替代 aria-label */}
...
<KeyboardEventsTable
  data={[
    ...
    { key: 'Home', description: '聚焦并激活第一个标签页' },
    { key: 'End', description: '聚焦并激活最后一个标签页' },
  ]}
/>
