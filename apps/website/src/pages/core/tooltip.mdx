import { TooltipDemos } from '@react-ui/docs-demos';
...
HTML 元素或包含 HTML 元素的 ref 对象。当你不将 tooltip 目标渲染为 JSX 元素时，
请使用 `target` 属性。
...
要启用多行模式，请设置 `multiline` 属性以启用换行，并使用 `w` [样式属性](/styles/style-props) 设置 tooltip 宽度：
...
Tooltip 基于 [Transition](/core/transition/) 组件构建；它支持 `transitionProps` 属性：
...
你可以通过设置 `openDelay` 和 `closeDelay` 属性（单位为毫秒）来延迟 tooltip 的打开/关闭事件：
