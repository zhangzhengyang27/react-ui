...
<InstallScript packages="@react-ui/ui dayjs" />
...
After installing the `@react-ui/ui` package and importing styles, you can use all components from it:

<Demo data={DatePickerInputDemos.usage} />

## Date values as strings

`@react-ui/ui` components work with date strings: `YYYY-MM-DD` or `YYYY-MM-DD HH:mm:ss` depending on the component. Those strings do not include any timezone-specific information.

## dayjs

`@react-ui/ui` components use [dayjs](https://day.js.org/) under the hood for date manipulations and formatting.
...
components exported from the `@react-ui/ui` package. `DatesProvider` supports the following settings:
