import { MonthPickerInputDemos } from '@react-ui/docs-demos';
import { Layout } from '@/layout';
import { MDX_DATA } from '@/mdx';

export default Layout(MDX_DATA.MonthPickerInput);

## MonthPicker 属性

`MonthPickerInput` 支持大部分 [MonthPicker](/dates/month-picker/) 的 props。
请阅读 [MonthPicker](/dates/month-picker/) 文档以了解本页未列出的所有组件功能。

## 用法

<Demo data={MonthPickerInputDemos.usage} />

## 多个日期

设置 `type="multiple"` 以允许用户选择多个日期：

<Demo data={MonthPickerInputDemos.multiple} />

## 日期范围

设置 `type="range"` 以允许用户选择日期范围：

<Demo data={MonthPickerInputDemos.range} />

## 预设

使用 `presets` prop 添加自定义月份预设。预设显示在日历旁边：

<Demo data={MonthPickerInputDemos.presets} />

要在 `type="range"` 时使用 `presets`，请将值定义为两个日期的元组：

<Demo data={MonthPickerInputDemos.presetsRange} />

## 在模态框中打开选择器

默认情况下，[MonthPicker](/dates/month-picker/) 在 [Popover](/core/popover/) 中渲染。
你可以通过设置 `dropdownType="modal"` 将其更改为 [Modal](/core/modal/)：

<Demo data={MonthPickerInputDemos.modal} />

## 值格式

使用 `valueFormat` prop 更改值标签的 [dayjs 格式](https://day.js.org/docs/en/display/format)：

<Demo data={MonthPickerInputDemos.valueFormat} />

## 值格式化器

`valueFormatter` 是 `valueFormat` prop 的更强大的替代方案。
它允许使用自定义函数格式化值标签。
该函数对所有组件类型（`default`、`multiple` 和 `range`）都相同——你需要在函数内部执行额外的检查来处理不同类型。

将自定义格式化函数与 `type="multiple"` 一起使用的示例：

<Demo data={MonthPickerInputDemos.valueFormatter} />

## 可清除

设置 `clearable` prop 在右侧区域显示清除按钮。注意如果设置了 `rightSection` prop，清除按钮将不会显示。

<Demo data={MonthPickerInputDemos.clearable} />

<ClearSectionMode />

<Demo data={MonthPickerInputDemos.clearSectionMode} />

## 禁用状态

<Demo data={MonthPickerInputDemos.disabled} />

## 最小和最大日期

`minDate` 和 `maxDate` prop 定义了可选的最小和最大日期。
你可以将 `minDate` 和 `maxDate` 指定为 `Date` 对象：

<Demo data={MonthPickerInputDemos.minMax} />

## 控制 props

`getYearControlProps` 和 `getMonthControlProps` prop 允许根据日期向控制组件传递 props。
它可用于禁用特定日期或自定义样式/className。

<Demo data={MonthPickerInputDemos.controlProps} />

## 标签格式

`decadeLabelFormat` 和 `yearLabelFormat` prop 允许更改标题中标签的格式。
这些 prop 接受 [dayjs 格式字符串](https://day.js.org/docs/en/display/format)。

<Demo data={MonthPickerInputDemos.labelFormat} />

## 列表格式

`monthsListFormat` 和 `yearsListFormat` prop 允许更改列表中月份和年份标签的格式。
这些 prop 接受 [dayjs 格式字符串](https://day.js.org/docs/en/display/format)。

<Demo data={MonthPickerInputDemos.listFormat} />

## 最大层级

`maxLevel` prop 用于设置点击标题中的标签可到达的最大层级。

<Demo data={MonthPickerInputDemos.maxLevel} />

## Input 属性

<InputFeatures component="MonthPickerInput" element="button" />

<Demo data={MonthPickerInputDemos.configurator} />

## 带图标

<Demo data={MonthPickerInputDemos.icon} />

<GetElementRef
  component="MonthPickerInput"
  refType="button"
  package="@react-ui/ui"
/>

<InputAccessibility
  component="MonthPickerInput"
  packageName="@react-ui/ui"
/>
