...
使用 [UIProvider](/theming/ui-provider/) 包裹应用：
...
如果你的应用使用服务端渲染，请在 `<head />` 中添加 [ColorSchemeScript](/theming/color-schemes) 并将 `uiHtmlProps` 展开到 `<html />` 元素上，以避免 hydration 警告：
...
- [postcss-preset-ui](/styles/postcss-preset) — 了解 postcss-preset-ui 的功能和混入
