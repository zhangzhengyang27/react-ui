...
with ReactUI's theming, styling, and other core features.
...
imported from `@react-ui/ui` package: `useStyles`, `createVarsResolver` and `factory` in the example above.
...
`ElementProps` reassigns `style` prop signature to make it compatible with ReactUI components and allow
CSS variables usage.
...
ReactUI components support namespace exports to group related types with the component.
