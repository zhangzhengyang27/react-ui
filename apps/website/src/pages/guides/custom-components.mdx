...
本指南将帮助你理解如何创建与 ReactUI 的主题、样式和其他核心功能集成的自定义组件。

`ExampleComponent` 将作为本指南中的示例：
...
[Box](/core/box) 组件是所有其他组件的基础。要创建自定义组件，请将其作为根元素，
并将 `...others` props 展开到它上面以支持[样式属性](/styles/style-props)。

要为组件添加[样式属性](/styles/style-props)类型，请继承 `BoxProps`。
...
`useProps` hook 用于支持[默认 props](/theming/default-props)。它接受以下参数：
- 组件名称，用于在 [theme](/theming/theme-object) 中引用组件
- 组件级别的默认 props
- 组件 props

`useProps` 按以下顺序合并 props：
1. 组件 props — 最高优先级
2. theme 上的[默认 props](/theming/default-props) — 较低优先级
...
// ✅ useProps 可以正确推断类型
// `radius` prop 是 `MantineRadius`
const defaultProps = {
  radius: 'md',
} satisfies Partial<ExampleComponentProps>;

// ❌ useProps 无法正确推断类型
// `radius` prop 是 `MantineRadius | undefined`
const defaultProps: Partial<ExampleComponentProps> = {
  radius: 'md',
};
```

你可以通过以下方式使用 [defaultProps](/theming/default-props)：
...
`useStyles` hook 用于支持 [Styles API](/styles/styles-api) 功能：
...
// 🔝 完整组件代码见上文
const getStyles = useStyles<ExampleComponentFactory>({
  // 组件名称，用于生成静态选择器（.mantine-ExampleComponent-root）
...
`varsResolver` 在 [Button](/core/button) 组件中的使用示例：
...
`polymorphicFactory` 用于创建[多态组件](/guides/polymorphic)。
如果你需要改变根元素，请使用 `polymorphicFactory` 替代 `factory`。
例如，[Button](/core/button) 组件就是多态的：默认根元素是 `button`，
...
例如，[Accordion](/core/accordion) 组件的 `value` 和 `onChange` props 类型
