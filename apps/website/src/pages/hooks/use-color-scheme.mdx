import { UseColorSchemeDemos } from '@react-ui/docs-demos';
...
`use-color-scheme` 底层使用 [use-media-query](/hooks/use-media-query/)。它依赖 `window.matchMedia()` [API](https://developer.mozilla.org/en-US/docs/Web/API/Window/matchMedia)，
