import { UseDebouncedStateDemos } from '@react-ui/docs-demos';
...
The `UseDebouncedStateOptions` and `UseDebouncedStateReturnValue` types are exported from the `@react-ui/hooks` package;
