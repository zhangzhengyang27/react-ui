import { UseFileDialogDemos } from '@react-ui/docs-demos';
...
`UseFileDialogOptions` and `UseFileDialogReturnValue` types are exported from the `@react-ui/hooks` package;
