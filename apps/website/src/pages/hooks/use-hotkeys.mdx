...
if you need to support other elements. For this purpose, the `@react-ui/hooks` package exports a `getHotkeyHandler` function
...
The `@react-ui/hooks` package exports `HotkeyItemOptions` and `HotkeyItem` types:
...
The `HotkeyItemOptions` and `HotkeyItem` types are exported from `@react-ui/hooks`;
