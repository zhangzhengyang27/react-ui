...
You can test this feature by opening 2 tabs with ReactUI docs side by side and changing the color scheme
...
`UseStorageOptions` and `UseStorageReturnValue` types are exported from the `@react-ui/hooks` package;
