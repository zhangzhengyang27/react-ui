import { SliderDemos, UseMoveDemos } from '@react-ui/docs-demos';
...
`@react-ui/hooks` exports `UseMovePosition` type, it can be used as a type parameter for `useState`:
...
`UseMovePosition`, `UseMoveReturnValue` and `UseMoveHandlers` types are exported from the `@react-ui/hooks` package;
