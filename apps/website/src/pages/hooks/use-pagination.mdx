import { PaginationDemos, UsePaginationDemos } from '@react-ui/docs-demos';
...
`UsePaginationOptions` and `UsePaginationReturnValue` types are exported from the `@react-ui/hooks` package;
