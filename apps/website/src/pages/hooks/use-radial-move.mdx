import { UseRadialMoveDemos } from '@react-ui/docs-demos';
...
`use-radial-move` Hook 可用于创建自定义径向滑块。例如，[AngleSlider](/core/angle-slider)
组件就是基于此 Hook。它的工作方式与 [use-move](/hooks/use-move) Hook 类似。
