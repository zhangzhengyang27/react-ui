import { UseScrollIntoViewDemos } from '@react-ui/docs-demos';
...
`UseScrollIntoViewOptions` and `UseScrollIntoViewReturnValue` types are exported from the `@react-ui/hooks` package;
