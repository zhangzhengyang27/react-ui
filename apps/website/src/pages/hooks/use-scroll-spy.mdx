import { UseScrollSpyDemos } from '@react-ui/docs-demos';
...
table of contents components (like in the react-ui.dev sidebar on the right side)
...
All types used in the definition are exported from `@react-ui/hooks` package.
...
`UseScrollSpyOptions` and `UseScrollSpyReturnValue` types are exported from the `@react-ui/hooks` package;
