import { ScrollerDemos, UseScrollerDemos } from '@react-ui/docs-demos';
...
`UseScrollerOptions`, `UseScrollerReturnValue` and `UseScrollerScrollState` types are exported from the `@react-ui/hooks` package;
