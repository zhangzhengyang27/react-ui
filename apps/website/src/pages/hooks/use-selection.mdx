import { UseSelectionDemos } from '@react-ui/docs-demos';
...
`UseSelectionInput`, `UseSelectionReturnValue` and `UseSelectionHandlers` types are exported from the `@react-ui/hooks` package;
