...
The `UseStateHistoryHandlers` and `UseStateHistoryValue` interfaces are exported from the `@react-ui/hooks`
package.
...
The `UseStateHistoryHandlers`, `UseStateHistoryReturnValue`, and `UseStateHistoryValue` types are exported from `@react-ui/hooks`;
