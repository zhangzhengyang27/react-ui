...
All `@react-ui/schedule` components accept events through the `events` prop. Each event
...
format (the `DateTimeStringValue` type used by all ReactUI date components).
...
These types are shared across `@react-ui/ui` and `@react-ui/schedule` and are exported
from both packages.
