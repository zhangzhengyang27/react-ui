...
All ReactUI components use CSS modules for styling.
It is recommended to use CSS modules in your project as well, but it is not required –
ReactUI components are fully compatible with any third-party styling solution and native CSS.
...
## Adding styles to ReactUI components

You can add styles to most ReactUI components using the `className` prop
– the same way as you would with a regular HTML element.
To set properties to your [theme](/theming/theme-object) values, you can use [ReactUI CSS variables](/styles/css-variables):

<Demo data={StylesDemos.className} />

To apply styles to inner elements of ReactUI components with CSS modules, you can use the `classNames` prop
(see [Styles API](/styles/styles-api) for more information):

<Demo data={StylesDemos.classNames} />

## Styling ReactUI components without CSS modules

All ReactUI components are fully compatible with any third-party styling solution and native CSS.
...
>
> CSS modules are the recommended way of styling ReactUI components.
...
> - It is impossible to style inner elements of ReactUI components with static selectors when using styled-components and other similar libraries if the component uses [Portal](/core/portal) because some elements will be rendered outside of the component root and inner elements are not part of the component tree
