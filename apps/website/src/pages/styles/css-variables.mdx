...
# ReactUI CSS variables

[MantineProvider](/theming/mantine-provider) exposes all ReactUI CSS variables based on the given [theme](/theming/theme-object).
...
of all ReactUI components.

### Font family

The following CSS variables are used to assign font families to all ReactUI components:
  ...
  data={[
    {
      ...
      description:
        'Controls the font-family property of most ReactUI components',
...
Font size variables are used in most ReactUI components to control text size. The
...
Some ReactUI components like [Button](/core/button) or [Badge](/core/badge) have `variant` prop
that in combination with `color` prop controls the component text, background and border colors.
For each variant and color, ReactUI defines a set of CSS variables that control these colors.
...
The following colors are used in various ReactUI components. Note that default values
...
`theme.spacing` values are used in most ReactUI components to control paddings, margins, and other
...
ReactUI components that support `radius` prop use border radius variables to control border radius.
...
Shadow variables are used in all ReactUI components that support `shadow` prop. The following CSS
...
z-index variables are defined in `@react-ui/ui/styles.css`. Unlike other variables,
...
modify values of ReactUI CSS variables or even add your own variables.
