...
The `@react-ui/ui` package includes some global styles that are required for components to work correctly.
...
The `@react-ui/ui` package includes minimal CSS reset – it includes only basic styles required for components to work
...
The `@react-ui/ui` package includes the following `body` and `:root` element styles:
...
The `@react-ui/ui` package includes the following static classes:
...
It is recommended to use [CSS modules](/styles/css-modules) to apply styles to ReactUI components
...
In global `.css` files you can reference all ReactUI [CSS variables](/styles/css-variables) and
