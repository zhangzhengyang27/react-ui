...
`postcss-preset-ui` 提供了多个 CSS 函数和 mixin，帮助你编写样式。
它不是必须使用的，但强烈推荐。所有涉及样式的 demo 都假设你已经安装了这个 preset。

`postcss-preset-ui` 包含以下 PostCSS 插件：
...
<InstallScript dev packages="postcss-preset-ui" />
...
你也可以将 `smaller-than` 和 `larger-than` mixins 与 [ui 断点](/styles/responsive/#breakpoints-variables-in-css-modules) 一起使用：
