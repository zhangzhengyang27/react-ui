...
可以在 [theme](/theming/theme-object) 上设置 `scale` 为 `1 / htmlFontSize`。
...
同样的转换也发生在 [style props](/styles/style-props/) 中：
...
如果已安装 [postcss-preset-mantine](/styles/postcss-preset)，你可以在 [css 文件](/styles/css-modules)中使用 `rem` 和 `em` 函数：
...
[postcss-preset-mantine](/styles/postcss-preset) 配置中启用 `autoRem` 选项：
