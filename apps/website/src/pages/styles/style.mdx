...
All ReactUI components that have a root element support the `style` prop.
...
as the React `style` prop. You can use ReactUI [CSS variables](/styles/css-variables) in the style object
...
You can define CSS variables in the style prop. Note that this only works with ReactUI components:
...
It is useful when you want to create a wrapper around a ReactUI component, add inline styles and keep the option to pass
