...
inside a ReactUI component – inline or using the [theme object](/theming/theme-object). All ReactUI components that
have styles support the Styles API.

## Styles API selectors

Every ReactUI component that supports the Styles API has a set of element names that can be used to
apply styles to inner elements inside the component. For simplicity, these element names are called
selectors in the ReactUI documentation. You can find selector information under the `Styles API` tab
...
With the `classNames` prop you can add classes to inner elements of ReactUI components. It accepts
...
Most of ReactUI components use CSS variables to define colors, sizes, paddings and other
...
You can use these classes to create components with the same styles as ReactUI components:
...
You can pass attributes to inner elements of ReactUI components using the `attributes` prop.
