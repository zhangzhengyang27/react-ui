import { ButtonDemos } from '@react-ui/docs-demos';
...
本指南将帮助你了解如何为 ReactUI 和自定义组件应用样式。

## 组件特定 props

大多数组件都提供允许你自定义样式的 props。例如，[Button](/core/button/) 组件具有 `color`、`variant`、`size` 和 `radius` props，用于控制其外观：
...
[Style props](/styles/style-props/) 的工作方式与组件特定 props 类似，但有以下几个区别：
...
当你需要更改单个 CSS 属性而无需为样式创建单独的文件时，[Style props](/styles/style-props/) 非常有用。一些最常见的用例包括：
...
注意，[style props](/styles/style-props/) 从未打算作为组件样式的主要方式。在大多数情况下，最好将每个组件使用的 style props 数量限制在 3-4 个以内。如果你发现自己使用了超过 4 个 style props，请考虑创建一个单独的样式文件——这样更容易维护，并且[性能更好](/styles/styles-performance/)。

## style 属性

所有 ReactUI 组件都支持 [style 属性](/styles/style/)，它允许设置 CSS 属性和 CSS 变量。它在以下情况下很有用：
...
[style 属性](/styles/style/) 的工作方式与 React 的 `style` prop 相同。不建议将其作为组件样式的主要方式。在大多数情况下，最好创建一个单独的样式文件——这样更容易维护，并且[性能更好](/styles/styles-performance/)。

## CSS 模块

[CSS modules](/styles/css-modules/) 是为 ReactUI 组件应用大多数样式的推荐方式。CSS modules 是样式化组件性能最好、最灵活的方式。
...
你可以在任何样式中通过 [CSS 变量](/styles/css-variables/) 引用 ReactUI [主题](/theming/theme-object/) 值：

- 在 [CSS modules](/styles/css-modules/) 中：
...
- 在 [style props](/styles/style-props/) 中：
...
function Demo() {
  ...
  // "red.5" 是 var(--mantine-color-red-5) 的简写

  // mt="xl" 引用 theme.spacing.xl
  // "xl" 是 var(--mantine-spacing-xl) 的简写
  return (
    <Box bg="red.5" mt="xl">
      My box
    </Box>
