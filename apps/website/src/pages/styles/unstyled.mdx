...
## Using ReactUI as a headless UI library

You can use ReactUI as a headless UI library. To do that, simply do not import `@mantine/*/styles.css`
in your application. Then you will be able to apply styles to ReactUI components using the [Styles API](/styles/styles-api/)
...
that should be used when you want to use ReactUI as a headless UI library. It removes all
features that are related to ReactUI styles:

- ReactUI classes are not applied to components
...
Most ReactUI components support the `unstyled` prop that removes library styles from the component and allows you to style it
...
> styled with ReactUI styles.
>
> Note that `unstyled` prop does not remove ReactUI library styles from your `.css` bundle –
