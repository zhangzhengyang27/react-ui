...
- With Vanilla extract you do not have access to [postcss-preset-mantine](/styles/postcss-preset) features like the `light-dark` function and `hover` mixin.
  Because of this, you will not be able to copy-paste all demos from the ReactUI documentation and use them with Vanilla extract.
...
Then install the `@mantine/vanilla-extract` package; it exports the `themeToVars` function to convert the ReactUI theme to CSS variables:
...
You should not use Vanilla extract `createTheme` to generate ReactUI theme tokens – all ReactUI [theme](/theming/theme-object)
properties are already exposed as CSS variables. Instead, use `themeToVars` function from `@mantine/vanilla-extract` package
to create an object with CSS variables from ReactUI theme:
...
Import `vars` object in `*.css.ts` files to access ReactUI [CSS variables](/styles/css-variables):
...
To convert px to [rem or em](/styles/rem) use `rem` and `em` functions from `@react-ui/ui` package:
