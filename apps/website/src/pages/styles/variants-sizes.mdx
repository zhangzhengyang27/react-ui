...
注意，你可以为任何支持 [Styles API](/styles/styles-api) 的 ReactUI 组件添加自定义变体，
即使库本身没有为该组件预定义变体。

> **覆盖已有变体样式**
>
> 除了添加新变体外，你还可以覆盖已有变体，例如使用 `.input[data-variant="filled"]` 选择器
> 来修改 [Input](/core/input) 组件的 `filled` 变体。

## 自定义变体类型

你可以通过在项目中创建 `mantine.d.ts` 文件，并用新的变体类型扩展 `{x}Props` 接口，
来为自定义变体定义类型。

为 [Button](/core/button) 组件添加自定义变体类型的示例：
...
[Button](/core/button)、[Badge](/core/badge)、[ActionIcon](/core/action-icon) 等
组件都支持通过 [variantColorResolver](/theming/colors#colors-variant-resolver) 使用自定义变体——
...
[ActionIcon](/core/action-icon) 组件有以下 CSS 变量：
...
你可以通过 [Styles API](/styles/styles-api) 覆盖这些值，或添加新的尺寸值：

<Demo data={ActionIconDemos.customSize} />

注意，部分组件的尺寸由多个 CSS 变量控制。例如，
[Button](/core/button) 组件有以下 CSS 变量：
...
通常，在这种情况下使用 `data-size` 属性或 [theme](/theming/theme-object) 上的 `vars` 会更加方便。
