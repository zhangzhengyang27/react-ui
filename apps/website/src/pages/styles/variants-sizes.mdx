...
Most ReactUI components support the `variant` prop. It can be used in CSS variables resolver,
...
Note that you can add custom variants to every ReactUI component that supports the [Styles API](/styles/styles-api),
...
ReactUI component sizes are defined with CSS variables (usually on the root element). For example,
