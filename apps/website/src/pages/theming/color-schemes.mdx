import { StylesDemos, ThemingDemos } from '@react-ui/docs-demos';
...
[MantineProvider](/theming/mantine-provider/) 管理应用中的配色方案上下文。你可以使用 `defaultColorScheme` 属性配置默认配色方案值；可选值为 `light`、`dark` 和 `auto`（使用系统配色方案）。默认值为 `light`。
...
当 [MantineProvider](/theming/mantine-provider/) 挂载时，它会在 `<html />` 元素上设置一个 `data-ui-color-scheme` 属性，值为用户之前选择的值或 `defaultColorScheme` 属性的值。`data-ui-color-scheme` 属性用于所有组件样式中，以确定每个组件应使用哪些颜色。

## use-mantine-color-scheme hook

`useMantineColorScheme` hook 可用于获取和设置当前配色方案值：
...
`useComputedColorScheme` 返回计算后的配色方案值——它返回 `light` 或 `dark`。它可用于实现配色方案切换逻辑：
...
默认情况下，在配色方案更改时会禁用所有元素上的过渡动画，以避免不一致的动画效果。要在配色方案更改期间启用过渡动画，请在 `useMantineColorScheme` hook 上设置 `keepTransitions: true` 选项：
...
如果你的应用中有服务端渲染（例如，如果你使用 [Next.js](/guides/next) 或 [React Router](/guides/react-router)），那么你不能在应用中使用 `colorScheme` 值以避免 hydration 问题。相反，你可以使用 [postcss-preset-mantine](/styles/postcss-preset) 中的 `dark` 和 `light` mixins 来生成根据配色方案值隐藏元素的样式：
...
`ColorSchemeScript` 组件会渲染一个 script 标签，在 hydration 之前将 `<html />` 元素上的 `data-ui-color-scheme` 属性设置为用户选择的值或 `defaultColorScheme` 属性的值。它用于避免服务端渲染应用中出现不准确的配色方案闪烁，例如 [Next.js](/guides/next) 或 [React Router](/guides/react-router)。请遵循特定框架的指南以了解在哪里渲染 `ColorSchemeScript` 组件。
...
在 `MantineProvider` 和 `ColorSchemeScript` 上设置 `defaultColorScheme="auto"` 以使用系统配色方案。在这种情况下，配色方案值将由用户的操作系统控制：
...
然后可以将自定义配色方案管理器传递给 [MantineProvider](/theming/mantine-provider)：
...
当用户尚未选择任何配色方案时，将使用默认配色方案值。它必须同时设置在 [MantineProvider](/theming/mantine-provider/) 和 `ColorSchemeScript` 上。如果未设置 `defaultColorScheme`，则使用 `light`。
...
你可以使用 `forceColorScheme` 属性将配色方案值强制为 `light` 或 `dark`。它必须同时设置在 [MantineProvider](/theming/mantine-provider/) 和 `ColorSchemeScript` 上。如果设置了 `forceColorScheme`，则 `defaultColorScheme` 和 `colorSchemeManager` 将被忽略。当设置了 `forceColorScheme` 时，无法使用 `setColorScheme` 函数更改配色方案值。
