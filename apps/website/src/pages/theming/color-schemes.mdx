...
## data-ui-color-scheme attribute

When [MantineProvider](/theming/mantine-provider/) is mounted, it sets a `data-ui-color-scheme`
...
The `data-ui-color-scheme` attribute is used in all component styles to determine which colors each component should use.
...
The `ColorSchemeScript` component renders a script tag that sets the `data-ui-color-scheme` attribute
...
All ReactUI components support `lightHidden` and `darkHidden` props that can be used to hide
...
If you need to support users with disabled JavaScript, you need to set the `data-ui-color-scheme`
