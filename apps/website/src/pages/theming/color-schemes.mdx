...
[UIProvider](/theming/ui-provider/) 管理应用中的配色方案上下文。你可以使用 `defaultColorScheme` 属性配置默认配色方案值；可选值为 `light`、`dark` 和 `auto`（使用系统配色方案）。默认值为 `light`。
...
当 [UIProvider](/theming/ui-provider/) 挂载时，它会在 `<html />` 元素上设置一个 `data-ui-color-scheme` 属性，值为用户之前选择的值或 `defaultColorScheme` 属性的值。`data-ui-color-scheme` 属性用于所有组件样式中，以确定每个组件应使用哪些颜色。
...
如果你的应用中有服务端渲染（例如，如果你使用 [Next.js](/guides/next) 或 [React Router](/guides/react-router)），那么你不能在应用中使用 `colorScheme` 值以避免 hydration 问题。相反，你可以使用 [postcss-preset-ui](/styles/postcss-preset) 中的 `dark` 和 `light` mixins 来生成根据配色方案值隐藏元素的样式：
...
然后可以将自定义配色方案管理器传递给 [UIProvider](/theming/ui-provider)：
...
当用户尚未选择任何配色方案时，将使用默认配色方案值。它必须同时设置在 [UIProvider](/theming/ui-provider/) 和 `ColorSchemeScript` 上。如果未设置 `defaultColorScheme`，则使用 `light`。
...
你可以使用 `forceColorScheme` 属性将配色方案值强制为 `light` 或 `dark`。它必须同时设置在 [UIProvider](/theming/ui-provider/) 和 `ColorSchemeScript` 上。如果设置了 `forceColorScheme`，则 `defaultColorScheme` 和 `colorSchemeManager` 将被忽略。当设置了 `forceColorScheme` 时，无法使用 `setColorScheme` 函数更改配色方案值。
