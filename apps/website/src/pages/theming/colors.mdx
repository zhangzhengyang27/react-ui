...
ReactUI uses [open-color](https://yeun.github.io/open-color/) in the default theme with some additions.
...
>
> You can add more than 10 shades per color: these values will not be used by ReactUI components with the default colors resolver,
...
- Transform dynamic string arrays to ReactUI color tuple (the array should still have 10 values)
...
or install the `@react-ui/colors-generator` package to generate dynamic colors in your application:
...
TypeScript will only autocomplete ReactUI's default colors when accessing the theme. To add your custom colors to the MantineColor type, you can use TypeScript module declaration.
