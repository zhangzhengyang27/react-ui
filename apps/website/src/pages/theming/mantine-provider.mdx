...
You can learn more about ReactUI CSS variables in the [CSS variables guide](/styles/css-variables/).
...
By default, it is set to `true`. If set to `false`, all ReactUI CSS variables will be added in a `<style />` tag
...
`getRootElement` is a function that returns the root application element (usually `html`) to set the `data-ui-color-scheme` attribute.
The default value is `() => document.documentElement` which means that the `data-ui-color-scheme`
...
You can learn more about ReactUI CSS variables in the [CSS variables guide](/styles/css-variables#css-variables-resolver).
