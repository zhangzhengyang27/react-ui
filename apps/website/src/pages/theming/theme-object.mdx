...
interface UITheme {
  ...
  fontWeights: UIFontWeightsValues;
...
要自定义主题，请将主题覆盖对象传递给 [UIProvider](/theming/ui-provider/) 的 `theme` 属性。主题覆盖对象会与默认主题进行深度合并。
...
`useUITheme` Hook 从 [UIProvider](/theming/ui-provider) 上下文中返回主题对象：
...
你可以从 `@react-ui/ui` 包中导入默认主题对象。它包含所有具有默认值的主题属性。当你将主题覆盖传递给 [UIProvider](/theming/ui-provider) 时，它会与默认主题进行深度合并。
