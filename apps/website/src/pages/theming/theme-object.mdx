...
ReactUI theme is an object where your application's colors, fonts, spacing, border-radius, and other design tokens are stored.
...
- All [@react-ui/ui](/dates/getting-started) components that are based on [Calendar](/dates/calendar) component
...
except for [Spotlight](/x/spotlight) and [@react-ui/ui](/dates/getting-started) components which only support the global theme setting.
...
For example, to revert the medium font weight from `600` back to `500` (the default in ReactUI 8):
...
You can import the default theme object from the `@react-ui/ui` package. It includes
