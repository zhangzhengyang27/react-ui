...
`theme.fontWeights` 控制所有组件中使用的 `font-weight` 值。默认值为 `regular: 400`、`medium: 600`、`bold: 700`。每个值都会映射到一个 CSS 变量：`--mantine-font-weight-regular`、`--mantine-font-weight-medium`、`--mantine-font-weight-bold`。
