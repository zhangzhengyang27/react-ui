...
export default Layout(MDX_DATA.UIProvider);

# UIProvider

`UIProvider` 提供 [theme object](/theming/theme-object) 上下文值，管理配色方案变更，并注入 [CSS 变量](/styles/css-variables/)。它必须渲染在应用的根节点，且应只使用一次。
...
`colorSchemeManager` 用于在外部存储中读取和设置配色方案值。默认情况下，`UIProvider` 使用 `window.localStorage` 存储配色方案值，但你也可以向 `colorSchemeManager` prop 传入自己的实现。你可以在 [配色方案指南](/theming/color-schemes) 中了解更多关于配色方案管理的内容。
...
`cssVariablesSelector` 是用于添加 [CSS 变量](/styles/css-variables/) 的 CSS 选择器。默认情况下，变量会应用到 `:root` 和 `:host`。`UIProvider` 会根据给定的 [theme override](/theming/theme-object/) 和 `cssVariablesResolver` 生成 CSS 变量，然后这些变量会渲染到应用旁边的 `<style />` 标签中。你可以在 [CSS 变量指南](/styles/css-variables/) 中了解更多关于 ReactUI CSS 变量的内容。
...
- `ui-Text-root` – 组件静态 class，属于 [Styles API](/styles/styles-api)
