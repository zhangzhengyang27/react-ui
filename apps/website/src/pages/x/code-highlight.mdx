import { CodeHighlightDemos } from '@react-ui/docs-demos';
...
`@react-ui/ui` package does not depend on any specific code highlighting library.
...
The Shiki adapter is used for all code highlighting in ReactUI documentation.
...
icons from the `@react-ui/dev-icons` package, but you can use any other icons library or custom
icons:
