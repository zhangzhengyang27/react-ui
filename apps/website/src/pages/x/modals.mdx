import { ModalsDemos } from '@react-ui/docs-demos';
...
- `cancelProps` – 取消按钮属性
- `confirmProps` – 确认按钮属性
- `groupProps` – 按钮 [Group](/core/group/) 属性
...
你可以通过将属性添加到每个 `modals.x` 函数的参数中，向下传递属性给 [Modal](/core/modal) 组件。以下示例设置 `radius`、`size` 和 `withCloseButton` 属性：
