...
It is used as a search on the react-ui.dev website; you can trigger it with the `Ctrl + K` shortcut.
...
The `@react-ui/ui` package exports an object with actions that can be used to control the spotlight:
...
You can also import actions directly from the `@react-ui/ui` package if you prefer this syntax:
