...
- `RichTextEditor.ColorPicker` – 允许从给定的预定义色板中拾取颜色，或使用 [ColorPicker](/core/color-picker/) 组件
...
默认情况下，`RichTextEditor` 使用 [Typography](/core/typography/) 和一些额外样式渲染内容。你可以通过设置 `withTypographyStyles={false}` 禁用这些样式：
...
然后你可以通过 [全局样式](/styles/global-styles/) 或 [Styles API](/styles/styles-api/) 添加自己的样式：
