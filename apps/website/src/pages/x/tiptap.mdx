...
<InstallScript packages="@react-ui/ui @react-ui/ui @react-ui/hooks @tiptap/react @tiptap/pm @tiptap/extension-link @tiptap/starter-kit" />
...
The `@react-ui/ui` package provides a UI for [Tiptap](https://tiptap.dev/). The `RichTextEditor` component
...
import { RichTextEditor as MantineRichTextEditor } from '@react-ui/ui';
...
The `@react-ui/ui` package provides a custom `Link` extension that is required to be used instead of
`@tiptap/extension-link` in order for the `Ctrl + K` keyboard shortcut to work:

```tsx
// Use Link extension exported from the @react-ui/ui package
import { useEditor } from '@tiptap/react';
...
control the `top` property with `stickyOffset`. For example, on the react-ui.dev documentation
...
```tsx
// RichTextEditorLabels type can be imported from @react-ui/ui package
...
Default labels (can be imported from `@react-ui/ui` package):
