...
function itHandlesMonthKeyboardEvents(options, name = "handles month arrow keyboard events correctly") {
  it(`${name} (numberOfColumns=1)`, async () => {
    const { container } = render(/* @__PURE__ */ jsx(options.component, { ...options.props, numberOfColumns: 1 }));
  ...
  it(`${name} (numberOfColumns=2)`, async () => {
    const { container } = render(/* @__PURE__ */ jsx(options.component, { ...options.props, numberOfColumns: 2 }));
    const months = container.querySelectorAll(`.ui-${options.name}-month`);
...
export { itHandlesMonthKeyboardEvents };
