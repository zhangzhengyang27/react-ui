...
import { ActionIcon, Checkbox, Combobox, Group, TextInput, useCombobox } from '@xiaoye-react/ui'
import classes from './TransferList.module.css'

const fruits = ['🍎 苹果', '🍌 香蕉', '🍓 草莓']

const vegetables = ['🥦 西兰花', '🥕 胡萝卜', '🥬 生菜']
                ...
                <Combobox.EventsTarget>
                    <Group wrap="nowrap" gap={0} className={classes.controls}>
                        ...
                        <ActionIcon
                            ...
                            aria-label={
                                type === 'forward' ? 'Transfer selected items right' : 'Transfer selected items left'
                            }
                            onClick={() => {
                                onTransfer(value)
                                setValue([])
...
export function TransferList() {
    ...
    return (
        <div className={classes.root}>
            <RenderList type="forward" options={data[0]} onTransfer={options => handleTransfer(0, options)} />
            <RenderList type="backward" options={data[1]} onTransfer={options => handleTransfer(1, options)} />
        </div>
    )
}
