...
export interface ComboboxChevronProps extends React.ComponentPropsWithoutRef<'svg'> {}
...
export const ComboboxChevron = factory<ComboboxChevronFactory>((_props, ref) => {
    const props = useProps('ComboboxChevron', null, _props)
    ...
    return (
        <svg
            ...
            className={[classes.chevron, className].filter(Boolean).join(' ')}
            style={style}
            {...others}
        >
            <path
