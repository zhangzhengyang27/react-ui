...
export function usePillsReorder<T>({ value, onChange, enabled }: UsePillsReorderInput<T>) {
  ...
  useEffect(() => {
    ...
    pending.container
      ?.querySelector<HTMLElement>(`[data-ui-pill-index="${pending.index}"]`)
      ?.focus();
  });
  ...
  const handleInputKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    const pills = container.querySelectorAll<HTMLElement>('[data-ui-pill-index]');
    if (pills.length === 0) {
      return;
    }
  ...
  const getPillProps = (index: number): PillReorderProps | undefined => {
    ...
    return {
      ...
      onKeyDown: (event) => {
        ...
        container.querySelector<HTMLElement>(`[data-ui-pill-index="${targetIndex}"]`)?.focus();
      },
    };
  };
