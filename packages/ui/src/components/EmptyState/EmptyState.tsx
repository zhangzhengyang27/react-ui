...
export interface EmptyStateProps extends BoxProps, StylesApiProps<EmptyStateFactory>, ElementProps<'div', 'title'> {
    ...
    /** Controls the indicator appearance @default undefined */
    ...
    /** If set, a neutral circular background is displayed behind the indicator @default false */
...
const varsResolver = createVarsResolver<EmptyStateFactory>((theme, { size, variant, color }) => {
    const colors = variant ? theme.variantColorResolver({ color: color || theme.primaryColor, theme, variant }) : null
...
export const EmptyState = factory<EmptyStateFactory>((_props, ref) => {
    ...
    return (
        <EmptyStateProvider value={{ getStyles, withIndicatorBackground: withIndicatorBackground || !!variant }}>
            <Box ref={ref} {...getStyles('root', { variant })} variant={variant} mod={[{ align }, mod]} {...others}>
                {indicator}
                {hasBody && (
                    <div {...getStyles('body')}>
