...
export function ModalStack({ children }: ModalStackProps) {
    ...
    // 身份稳定：id 已存在时返回原数组引用。否则 Modal 挂载 effect 的重复注册
    // 会永远产生新数组身份 → Stack 重渲染 → context value 更新 → Modal effect 重跑，
    ...
    // 作为 consumer 的 Modal 其 effect 依赖 stackCtx，会反复 cleanup/setup 陷入无限循环
