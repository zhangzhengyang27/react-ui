...
const varsResolver = createVarsResolver<SpoilerFactory>((_, { transitionDuration }) => ({
    root: {
        '--spoiler-transition-duration': transitionDuration !== undefined ? `${transitionDuration}ms` : undefined
    }
}))
...
export const Spoiler = factory<SpoilerFactory>((_props, ref) => {
    ...
    return (
        <Box ref={ref} {...getStyles('root')} id={_id} data-has-spoiler={spoiler || undefined} {...others}>
            {spoiler && (
                <Anchor
                    component="button"
                    type="button"
